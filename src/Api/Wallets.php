...
class Wallets
{
    ...
    public function addInternalWalletAsset(string $walletId, string $assetId, string $address, ?string $tag = null): array { $body = ['address' => $address]; if ($tag !== null && $tag !== '') { $body['tag'] = $tag; } return $this->client->post("/v1/internal_wallets/{$walletId}/{$assetId}", $body); }
    ...
    public function addExternalWalletAsset(string $walletId, string $assetId, string $address, ?string $tag = null, ?array $additionalInfo = null): array { $body = ['address' => $address]; if ($tag !== null && $tag !== '') { $body['tag'] = $tag; } if ($additionalInfo !== null && count($additionalInfo) > 0) { $body['additionalInfo'] = $additionalInfo; } return $this->client->post("/v1/external_wallets/{$walletId}/{$assetId}", $body); }
